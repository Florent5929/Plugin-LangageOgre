...
public class Langage implements Listener {
	...
	int NumFirstCharMessageSansPrefixe;
		...
		if(main.pseudolistogres.contains(p.getName()) &&
				!Prefixe.equals("$") &&
				!Prefixe.equals(":") &&
				!Prefixe.equals("*") &&
				!Prefixe.equals("(")
...
// Le cas échéant, on réaffecte à Prefixe une chaîne vide.
			...
			for (Player t : e.getRecipients()) {
				if(main.pseudolistmages.contains(t.getName()) || 
				main.pseudolistogres.contains(t.getName()) ||                          
				t.isOp()){
        ...
        // Si le message se terminait par un ! ou un ?, on range ce ! ou ce ? dans la variable ponctuation.
			...
			String fin = " ";
			int col = main.TableauTrad[0].length;
			String motcompare = "";
			
			for(int j = 0; j <= nbmots-1; j++){ // Pour chaque mot du message...
				for(int i = 0; i <= col-1; i++){ // ... et pour chaque mot de français répertorié dans TableauTrad...
					
					// ... on va essayer de voir si le mot du message n'est pas répertorié dans TableauTrad
					...
					if(motcompare.equalsIgnoreCase(main.TableauTrad[0][i])) 
						// Si on trouve le mot dans la liste de mots de français de TableauTrad...
					{ parts[j] = main.TableauTrad[1][i];
					...
					parts[j] = main.TableauMots[r.nextInt(main.TableauMots.length -1)];
		...
		} // fin du if(main.pseudolistogres.contains(p.getName()) && [...]
